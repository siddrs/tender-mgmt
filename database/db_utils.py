    ...
    if st.button("Award Tender"):
        try:
            ...
            try:
                ...
                cur.execute(
                    "INSERT INTO Notification (vendor_id, title, message) VALUES (?, ?, ?)",
                    (winner_id, f":green[Tender Awarded]", f":green[Congratulations. You have been awarded tender {selected_ref}.]")
                )
                ...
                for vid in bidder_ids:
                    if vid != winner_id:
                        cur.execute(
                            "INSERT INTO Notification (vendor_id, title, message) VALUES (?, ?, ?)",
                            (vid, f":red[Tender Result]", f":red[Sorry, You weren't awarded with the tender {selected_ref}. It has been awarded to a different vendor.]")
                        )
            except Exception:
                pass
